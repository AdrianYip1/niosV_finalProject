...
OUTPUT_C_FILE = "titleScreen_frames.c"
OUTPUT_H_FILE = "titleScreen_frames.h"
TILE_SIZE_DEFINE = "TITLE_SCREEN_TILE_SIZE"
SPRITE_PREFIX = "titleScreen_frame_"
...
def write_header(header_name, frame_count): #automatically writes the header file for the sprite
    with open(header_name, "w") as h:
        h.write("#pragma once\n\n")
        h.write(f"#define TITLE_SCREEN_TILE_SIZE {TILE_SIZE}\n") 
        h.write(f"#define TITLE_SCREEN_FRAME_COUNT {frame_count}\n\n")

        for i in range(frame_count):
            h.write(
                f"extern const unsigned short titleScreen_frame_{i}[TILE_SIZE * TILE_SIZE];\n"
            )

        h.write(
            "\nextern const unsigned short* const titleScreenFrames[TITLE_SCREEN_FRAME_COUNT];\n"
        )
...
if __name__ == "__main__":  # loop through every single frame_#.png file, deletes the pngs and makes the h adn c files
    ...
    with open(OUTPUT_C_FILE, "w") as out:
        ...
        if frame_count > 0:
            out.write(
                f"const unsigned short* const titleScreenFrames[TITLE_SCREEN_FRAME_COUNT] = {{\n"
            )
            for i in range(frame_count):
