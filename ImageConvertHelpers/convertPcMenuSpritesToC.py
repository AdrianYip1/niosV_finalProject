...
SRC_DIR = Path("ImageConvertHelpers/pcMenuSprites")

OUT_DIR = Path("software/graphics/sprites/pcMenu")
OUT_C = OUT_DIR / "pcMenuSprites.c"
OUT_H = OUT_DIR / "pcMenuSprites.h"
...
def main() -> None:
    ...
    # Keep stable ordering + names.
    sprites: list[tuple[str, Path]] = []
    for p in pngs:
        sprites.append((f"{p.stem}Sprite", p))

    OUT_DIR.mkdir(parents=True, exist_ok=True)

    # Header
    h_lines: list[str] = ["#pragma once", "", "#include <stdint.h>", ""]
    h_lines.append(f"#define PC_MENU_SPRITE_COUNT  {len(sprites)}")
    h_lines.append("")
    h_lines.append("typedef enum {")
    for i, (name, _) in enumerate(sprites):
        enum_base = name[: -len("Sprite")] if name.endswith("Sprite") else name
        h_lines.append(f"    PC_MENU_SPRITE_{camel_to_screaming_snake(enum_base)} = {i},")
    h_lines.append("} PcMenuSpriteId;")
    h_lines.append("")
    ...
    for name, path in sprites:
        ...
        macro_base = f"PC_MENU_{camel_to_screaming_snake(name[: -len('Sprite')] if name.endswith('Sprite') else name)}"
    ...
    c_lines.append("const unsigned short* const pcMenuSprites[PC_MENU_SPRITE_COUNT] = {")
    for i, (name, _) in enumerate(sprites):
    ...
    c_lines.append("const uint16_t pcMenuSpriteWidths[PC_MENU_SPRITE_COUNT] = {")
    ...
    c_lines.append("const uint16_t pcMenuSpriteHeights[PC_MENU_SPRITE_COUNT] = {")
